...
- **When generating into an empty pattern, write through the END of the pattern**
  (line 255 at 256 lines). Do not stop after one bar; add variation (velocity,
  density, fills) across the full length. If a line count is explicitly given, follow it.
...
- **注意**: これらの名前は命名規則であり、OSC の instrument 引数には **Renoise 上の実際の楽器名** を使うこと。
  現在のテンプレートの楽器名: Kick Generator, Break - Bangy Bangy, Diode 03, Tension, Lucid Dream, Arp Saw Square, Harsh Noise, tv_set_mono
