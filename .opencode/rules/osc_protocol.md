...
> **Type constraint**: `tools/AIDJ/osc_protocol.lua` decodes **only `i` (int32) and `s` (string)** (OSC 1.0). `message_queue.py` / bridge は整数値floatのみintへ正規化し、非整数floatや他の型を送信前に拒否する。連続値はint×1000で送る。
...
## Status broadcast (Renoise Tool -> osc_bridge.py, one OSC message, ~10 Hz)
| OSC path                | args                                                   |
|-------------------------|--------------------------------------------------------|
...
- The validator returns non-zero on any of:
  ...
  - `require` of an `http` module (string-match only)
- On non-zero exit, the agent MUST NOT send the OSC dispatch; it should fix and retry once.

## Payload format for `osc_bridge.py`
opencode publishes one atomic JSON file per OSC message through
`host.osc.message_queue.queue_message()`:
```jsonc
{
  "id": "uuid",
  "ts": 1719344400123,
...
`osc_bridge.py`はoutboxをalphabetical orderで消費し、送信後`host/osc/sent/`へ移動する。
